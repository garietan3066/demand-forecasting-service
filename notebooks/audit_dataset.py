...
def inspect_batch(table):
    ...
    for name in arrays:
        ...
        matrices[name] = (
            pc.list_flatten(pc.filter(col, pa.array(valid)))
            .to_numpy(zero_copy_only=False)
            .reshape(-1, 24)
        )
        masks[name] = valid
    valid = masks["hours_sale"]
    counts["daily_hourly_sales_mismatch"] = int(
        (
            ~np.isclose(
                matrices["hours_sale"].sum(axis=1),
                frame.sale_amount.to_numpy()[valid],
                atol=1e-6,
                rtol=1e-6,
            )
        ).sum()
    ...
    counts["stock_count_mismatch_zeros_06_22"] = int(
        ((stock[:, 6:22] == 0).sum(axis=1) != recorded).sum()
    )
    ...
    if np.array_equal(masks["hours_sale"], valid):
        counts["positive_sales_hours_flagged_stockout"] = int(
            ((matrices["hours_sale"] > 0) & (stock == 1)).sum()
        )
...
def summarize_series(frame):
    ...
    return {
        ...
        "store_product_series": len(groups),
        "date_min": str(unique.dt.min().date()),
        "date_max": str(unique.dt.max().date()),
        ...
        "observations_per_series": {
            str(k): int(v) for k, v in groups["count"].value_counts().sort_index().items()
        },
        "series_start_dates": {
            str(k.date()): int(v) for k, v in groups["min"].value_counts().items()
        },
        "series_end_dates": {
            str(k.date()): int(v) for k, v in groups["max"].value_counts().items()
        },
        "sales_quantiles": {
            str(k): float(v)
            for k, v in frame.sale_amount.quantile([0, 0.25, 0.5, 0.75, 0.95, 0.99, 1]).items()
        },
        "zero_sales_by_stockout": {
            label: {
                "rows": int(mask.sum()),
                "zero_sales_rows": int((mask & frame.sale_amount.eq(0)).sum()),
            }
            for label, mask in {
...
def main():
    ...
    if not shards or len({p.parent for p in shards}) != 1:
        raise SystemExit(
            "Expected one cached training revision. Run inspect_dataset.py first or specify --cache-dir."
        )
        ...
        with pa.memory_map(str(path), "r") as source:
            for batch in pa.ipc.open_stream(source):
                ...
                if len(discount_examples) < 40:
                    discounts = table.select(
                        ["store_id", "product_id", "dt", "discount"]
                    ).to_pandas()
                    discount_examples.extend(
                        discounts.loc[~discounts.discount.between(0, 1)]
                        .head(40 - len(discount_examples))
                        .to_dict("records")
                    )
    ...
    report = {
        ...
        "split": "train",
        "cache_revision": shards[0].parent.name,
        "sources": sources,
